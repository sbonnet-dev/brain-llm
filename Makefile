...
.PHONY: help venv install run dev test lint clean \
        migrate migrate-revision migrate-stamp migrate-down migrate-history \
...
migrate-revision:  ## Generate a new autogenerated migration. Usage: make migrate-revision M="add foo"
	@if [ -z "$(M)" ]; then echo "Usage: make migrate-revision M=\"description\""; exit 1; fi
	$(VENV_BIN)/alembic revision --autogenerate -m "$(M)"

migrate-stamp:  ## Mark the DB as up-to-date without running migrations (use after manual schema fixes).
	$(VENV_BIN)/alembic stamp head
